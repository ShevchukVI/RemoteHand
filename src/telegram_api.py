...
class TelegramAPI:
    ...
    def send_message(self, text, parse_mode="HTML"):
        ...
        if not self.api_url:
            # (ВИПРАВЛЕНО) Додаємо логування, якщо токени не були знайдені
            logger.error("❌ Telegram не налаштовано. Повідомлення не відправлено.")
            return False
    ...
    def send_network_report(self, store_location, pc_name, test_results, user_name=""):
        """(ОНОВЛЕНО) Відправити звіт про стан мережі з user_name"""
